...
/**
 * Retrieves all currently valid RESILINK offers from ODEP.
 * Filters out expired offers and those without remaining quantity, enriches each offer
 * with the offerer's phone number, and synchronizes results with the RESILINK database.
 *
 * @param {string} url - Base ODEP API URL for offer endpoints
 * @param {string} token - Bearer JWT authorization token
 * @returns {Promise<Array>} - [validOffersMap{}, statusCode] keyed by offer ID
 */
...
/**
 * Retrieves a selection of randomly suggested valid RESILINK offers from ODEP.
 * Excludes offers from the requesting prosumer and offers they have already blocked.
 * Returns up to 3 randomly selected valid offers along with their associated asset data.
 *
 * @param {string} url - Base ODEP API URL for offer endpoints
 * @param {string} owner - Username of the requesting prosumer (used to exclude own offers)
 * @param {string} token - Bearer JWT authorization token
 * @returns {Promise<Array>} - [{offers[], assets{}}, statusCode] tuple
 */
...
/**
 * Retrieves a paginated batch of valid RESILINK offers from ODEP, ordered from newest to oldest.
 * Used to implement infinite-scroll or paginated offer listings on the platform.
 *
 * @param {string} url - Base ODEP API URL for offer endpoints
 * @param {number} offerNbr - Number of offers to return per page
 * @param {number} iteration - Page index (0-based) to determine the slice of offers to return
 * @param {string} token - Bearer JWT authorization token
 * @returns {Promise<Array>} - [{offers[], assets{}}, statusCode] tuple
 */
...
/**
 * Retrieves all RESILINK offers that are in a prosumer's blocked list.
 * Used to display a prosumer's hidden/blocked offers for management purposes.
 *
 * @param {string} url - Base ODEP API URL for offer endpoints
 * @param {string} owner - Username of the prosumer whose blocked offers to retrieve
 * @param {string} token - Bearer JWT authorization token
 * @returns {Promise<Array>} - [{offers[], assets{}}, statusCode] tuple
 */
...
/**
 * Filters valid RESILINK offers based on a set of optional criteria.
 * Supports filtering by assetType, properties/specificAttributes, GPS perimeter, city/village,
 * name, country, transactionType, price range, quantity range, and date range.
 *
 * @param {string} url - Base ODEP API URL for offer endpoints
 * @param {Object|null} filter - Map of filter criteria (null to return all valid offers)
 * @param {string} token - Bearer JWT authorization token
 * @returns {Promise<Array>} - [filteredOffers[], statusCode] tuple as JSON string
 */
...
/**
 * Retrieves all ongoing purchase contracts for the authenticated user, enriched with
 * offer and asset data. Used to display the user's active transactions history.
 * Fetches contracts from ODEP and cross-references offer and asset maps for full context.
 *
 * @param {string} url - Base ODEP API URL for offer endpoints
 * @param {string} token - Bearer JWT authorization token
 * @returns {Promise<Array>} - [{contracts[], offers[], assets[]}, statusCode] tuple
 */
...
/**
 * Retrieves all offers created by the authenticated user from ODEP.
 * Synchronizes the retrieved offer data with the RESILINK database.
 *
 * @param {string} url - Base ODEP API URL for offer endpoints
 * @param {string} token - Bearer JWT authorization token
 * @returns {Promise<Array>} - [ownerOffers[], statusCode] tuple
 */
...
/**
 * Creates a new offer in ODEP and registers RESILINK-specific metadata in the local database.
 * Validates the transactionType before posting. Strips RESILINK-specific fields (transactionType,
 * country) from the body before forwarding to ODEP.
 *
 * @param {string} url - Base ODEP API URL for offer endpoints
 * @param {Object} body - Offer creation payload including transactionType and country fields
 * @param {string} token - Bearer JWT authorization token
 * @returns {Promise<Array>} - [createdOffer, statusCode] tuple
 */
...
/**
 * Creates both an asset and its associated offer in a single transaction.
 * First creates the asset via AssetService, then uses the returned assetId to create the offer.
 *
 * @param {string} url - Base ODEP API URL for offer endpoints
 * @param {Object} body - Combined payload with 'asset' and 'offer' sub-objects
 * @param {string} token - Bearer JWT authorization token
 * @returns {Promise<Array>} - [{asset, offer}, statusCode] tuple
 */
...
/**
 * Retrieves all offers registered in ODEP and synchronizes them with the RESILINK database.
 * Used for admin views and bulk offer management operations.
 *
 * @param {string} url - Base ODEP API URL for offer endpoints
 * @param {string} token - Bearer JWT authorization token
 * @returns {Promise<Array>} - [offers[], statusCode] tuple
 */
...
/**
 * Retrieves all offers from ODEP in a key-value map format for efficient internal lookups.
 * Used internally when cross-referencing offers from contract or purchase data.
 *
 * @param {string} url - Base ODEP API URL for offer endpoints
 * @param {string} token - Bearer JWT authorization token
 * @returns {Promise<Array>} - [offersMap{}, statusCode] keyed by offer ID as string
 */
...
/**
 * Retrieves detailed information for a specific offer from ODEP.
 * Synchronizes the offer data with the RESILINK database after retrieval.
 *
 * @param {string} url - Base ODEP API URL for offer endpoints
...
/**
 * Updates an existing offer by ID in ODEP and synchronizes changes with the RESILINK database.
 * Validates transactionType and strips RESILINK-specific fields before forwarding.
 *
 * @param {string} url - Base ODEP API URL for offer endpoints
 * @param {Object} body - Updated offer fields including transactionType and country
...
/**
 * Updates both an offer and its linked asset simultaneously in ODEP.
 * Delegates the asset update to AssetService before updating the offer.
 *
 * @param {string} url - Base ODEP API URL for offer endpoints
 * @param {Object} body - Combined payload with 'offer' and 'asset' sub-objects
 * @param {string} id - Offer identifier to update
 * @param {string} token - Bearer JWT authorization token
 * @returns {Promise<Array>} - [updateResponse, statusCode] tuple
 */
...
/**
 * Permanently deletes an offer by ID from ODEP and removes it from the RESILINK database.
 * Should be called only when there are no active contracts referencing this offer.
 *
 * @param {string} url - Base ODEP API URL for offer endpoints
